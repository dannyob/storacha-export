...
### Task 14: Manual integration test against real Storacha + production

This task requires Danny's Storacha credentials and SSH access to production.
...
- [ ] **Step 3: Test kubo export to production**

```bash
node bin/storacha-export.js --space Demos --backend kubo --kubo-api http://production:5001
```

Verify: CID appears pinned on production (`ssh production ipfs pin ls | grep <cid>`)
