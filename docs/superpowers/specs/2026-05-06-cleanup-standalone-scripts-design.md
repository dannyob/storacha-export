...
The production deployment tracks the old code; it remains untouched. This branch produces a clean future state for new work.
...
`storacha-download.mts` is not unit-tested. It's hard to test (auth, network, Storacha indexing service) and battle-tested in production via the 36k-uploads run on production.
...
- Migrating the running production deployment. production keeps tracking the old checkout. This branch is a future state for new work, not a hot-swap.
- Refactoring or testing `storacha-dashboard.mts`. The dashboard is being deleted in this branch; the running instance on production is unaffected.
